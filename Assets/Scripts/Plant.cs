...
public class Plant : Organism
{
    ...
    protected void SetOnTerrain(Transform transform, float offset=0)
    {
        ...
        // Above is the old code that didnt work ideally and spawned everything basically on one of two y levels

        RaycastHit hit;
        Vector3 pos = transform.position;

        if (Physics.Raycast(new Vector3(pos.x, 10f, pos.z), Vector3.down, out hit, 10f, terrainLayer)) {
